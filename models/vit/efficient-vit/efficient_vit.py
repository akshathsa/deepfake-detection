...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(), # could use nn.SiLU instead
            nn.Dropout(dropout),
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Imagine q represents queries and k represents keys in an attention layer.
        # This code calculates how relevant each key is to each query, considering their 
        # features and the head they belong to. The scaling factor adjusts the strength 
        # of these relevance scores.
...
# EfficientNet processes input faces + ViT processes every feature after linear projection
# CLS token produces binary classification score
class EfficientViT(nn.Module): 
    def __init__(self, config, channels=512, selected_efficient_net = 0):
        ...
        if selected_efficient_net == 0:
            self.efficient_net = EfficientNet.from_pretrained('efficientnet-b0') # EfficientNet B0 (smallest)
        else:
